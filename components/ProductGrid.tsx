    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
      ...
      {productos.map((producto) => (
        ...
        <Card key={producto.id}
          className="group overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 border p-0 gap-0">
          ...
          <CardContent className="p-4 border space-y-3">
            
              <Link href={`/producto/${producto.id}`} >
                <h3 className="font-semibold text-foreground border ">
                  {producto.titulo}
                </h3>
              </Link>
              ...
              <div className="flex items-center  space-y-1 border">

                  <p className="text-2xl font-bold text-primary">${producto.precio}</p>
                  <p className="text-sm text-muted-foreground line-through">
                    ${(Number.parseFloat(producto.precio) * 1.2).toFixed(2)}</p>

              </div>


              <div className="flex items-center justify-between mt-2 border">
